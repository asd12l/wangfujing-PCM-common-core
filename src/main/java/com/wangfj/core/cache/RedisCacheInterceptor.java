...
public class RedisCacheInterceptor implements AfterReturningAdvice {
    ...
    @Override
    public void afterReturning(Object result, Method method, Object[] parameter, Object target)
            throws Throwable {
        String flag = "";
        if (parameter.length > 0) {
            if (parameter[0].getClass().isAssignableFrom(RedisCacheBean.class)) {
                ...
                if (!redisCacheBean.isEmpty()) {
                    ...
                    try {
                        if (ADD.equals(flag)) {
                            redisUtil.setHSet(redisCacheBean.getRedisName(),
                                    redisCacheBean.getRedisKey(),
                                    JsonUtil.getJSONString(redisCacheBean.getObj()));
                        } else if (UPDATE.equals(flag)) {
                            redisUtil.delHSet(redisCacheBean.getRedisName(),
                                    redisCacheBean.getRedisKey());
                            redisUtil.setHSet(redisCacheBean.getRedisName(),
                                    redisCacheBean.getRedisKey(),
                                    JsonUtil.getJSONString(redisCacheBean.getObj()));
